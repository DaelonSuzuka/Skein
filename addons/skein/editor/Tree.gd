...
func _refresh():
	...
	for data in file_data:
		...
		var nodes = Skein.load_conversation(data.path, {})
...
func create_folder_item(parent, path):
	...
	item.set_text(0, path.get_file())
	item.set_meta('name', path.get_file())
	item.set_tooltip(0, path.trim_prefix(Skein.conversation_prefix))
	return item
...
func context_menu_item_selected(selection: String) -> void:
	match selection:
		'Convert to Yarn':
			var item = get_selected()
			var path = item.get_meta('path')
			var nodes = Skein.load_conversation(path)
			if !ge:
				ge = load('res://addons/skein/editor/GraphEdit.gd').new()
				add_child(ge)
			ge.set_nodes(nodes)
			nodes = ge.get_nodes()
			if nodes:
		...
		'Copy Path':
			...
			OS.clipboard = path.trim_prefix(Skein.conversation_prefix)
		'Rename':
			_start_rename()
		'Run':
